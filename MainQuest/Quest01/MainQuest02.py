...
if __name__ == "__main__":
    ...
    # 플레이어 생성
    player_name = input("영웅의 이름을 입력하세요: ")
    player = Character(player_name, level=1, hp=100, attack=20, defense=5)
    
    # 각 몬스터와 순차적으로 전투
    for monster_name, monster_level in monster_dict.items():
        # 몬스터 레벨에 따른 능력치 조정
        monster_hp = 50 * monster_level
        monster_attack = 10 * monster_level
        monster_defense = 2 * monster_level
        
        # 몬스터 생성
        monster = Character(monster_name, level=monster_level, 
                          hp=monster_hp, attack=monster_attack, 
